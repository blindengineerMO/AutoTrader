...
const textVector = require('./textVector');

log.setLevel(LogLevel.ERROR);

// Standalone node-client config shim — this package runs on a third-party
// operator's machine with no access to the coordinator's src/config, so the
// two touchpoints the crawler actually needs (search provider allow-list and
// retry backoff) are read from env vars here instead, matching the defaults
// in the coordinator's src/config/index.js.
const config = {
  search: {
    providers: process.env.SEARCH_PROVIDERS
      ? process.env.SEARCH_PROVIDERS.split(',').map((value) => value.trim()).filter(Boolean)
      : ['google', 'google-news', 'bing', 'mojeek', 'dogpile', 'duckduckgo-html'],
  },
  rateLimits: {
    retry: {
      baseDelayMs: Number(process.env.HTTP_RETRY_BASE_DELAY_MS) || 500,
    },
  },
};
